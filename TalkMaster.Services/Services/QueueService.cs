...
namespace TalkMaster.Services
{
    public class QueueService
    {
        ...
        {
            if (!Queue.Contains(userName)) return new UpdateResponse() { IsSuccessfull = false, UpdatedContent = null, UpdateType = UpdateType.Leaving };

            Queue = new Queue<string>(Queue.Where(user => user != userName));

            return new UpdateResponse() { IsSuccessfull = true, UpdatedContent = Queue.ToArray(), UpdateType = UpdateType.Leaving };
        }
        public CollectionResponse QWho(int count)
        {
            if (count == 0) return new CollectionResponse() { Count = Queue.Count, Names = Queue.ToArray() };
            else return new CollectionResponse { Count = count, Names = Queue.ToList().GetRange(0, count).ToArray(), Type = CollectionType.QWho};
        }
        public CollectionResponse QNext(int count)
        {
            ...
            return new CollectionResponse() { Count = list.Count, Names = list.ToArray(), Type = CollectionType.QNext};
        }
        public CollectionResponse LastQ()
        {
            ...
            return new CollectionResponse() { Count = _lastQueue.Count, Names = _lastQueue.ToArray(),  Type = CollectionType.LastQ};
        }
        public UpdateResponse Promote(string userName)
        {
            ...
            return new UpdateResponse() { IsSuccessfull = true, UpdatedContent = QWho(0), UpdateType = UpdateType.Promote };
        ...
        public UpdateResponse Clear()
        {
            ...
            return new UpdateResponse() { IsSuccessfull = true, UpdatedContent = Queue.ToArray(), UpdateType = UpdateType.Clear };
        }
        public UpdateResponse RemoveAtIndex(int index)
        {
            ...
            return new UpdateResponse() { IsSuccessfull = true, UpdatedContent = Queue.ToArray(), UpdateType = UpdateType.RemoveAt };
        }
